...
class VotingSession(SyncableMixin, Base):
    ...
    session_id: Mapped[int | None] = mapped_column(ForeignKey("legislative_sessions.id", ondelete="SET NULL"))
    ...
    voting_type: Mapped[str] = mapped_column(String(20), nullable=False, default="general")
    ...
    session: Mapped[LegislativeSession | None] = relationship(back_populates="voting_sessions")
...
class Vote(SyncableMixin, Base):
    __tablename__ = "votes"
    __table_args__ = (
        UniqueConstraint("voting_session_id", "legislator_id", name="uq_votes_session_legislator"),
    )

    voting_session_id: Mapped[int] = mapped_column(ForeignKey("voting_sessions.id", ondelete="CASCADE"), nullable=False)
...
class LegislatorVotingStats(SyncableMixin, Base):
    ...
    legislator_id: Mapped[int] = mapped_column(ForeignKey("legislators.id", ondelete="CASCADE"), nullable=False, unique=True)
    total_sessions: Mapped[int] = mapped_column(nullable=False, default=0)
    votes_for: Mapped[int] = mapped_column(nullable=False, default=0)
    votes_against: Mapped[int] = mapped_column(nullable=False, default=0)
    abstentions: Mapped[int] = mapped_column(nullable=False, default=0)
    absences: Mapped[int] = mapped_column(nullable=False, default=0)
    attendance_percentage: Mapped[float] = mapped_column(Numeric(5, 2), nullable=False, default=0)
    participation_rate: Mapped[float] = mapped_column(Numeric(5, 2), nullable=False, default=0)
