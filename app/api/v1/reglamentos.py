...
from app.schemas.reglamentos import ReglamentoDetail, ReglamentoStats, ReglamentoTimeline, ReglamentosResponse
...
def list_reglamentos(
    categoria: str | None = Query(None, description="en_tramite, tramitados, retirados"),
    ministerio: str | None = Query(None, description="Partial match on ministerio"),
    subsecretaria: str | None = Query(None, description="Partial match on subsecretaría"),
    ...
    reingresado: bool | None = Query(None, description="Filter by reingresado (retirado y luego reingresado)"),
    gobierno_actual: bool | None = Query(None, description="Only reglamentos with etapa activity from 2026-03-11 onwards"),
    offset: int = Query(reglamentos_service.DEFAULT_OFFSET, ge=0),
    limit: int = Query(reglamentos_service.DEFAULT_LIMIT, ge=1, le=reglamentos_service.MAX_LIMIT),
...
def reglamentos_stats_por_ministerio(
    ...
    return reglamentos_service.reglamentos_stats_por_ministerio(db=db, categoria=categoria)
...
def reglamentos_tiempo_tramitacion(
    ...
    return reglamentos_service.reglamentos_tiempo_tramitacion(db=db, categoria=categoria, limit=limit)
