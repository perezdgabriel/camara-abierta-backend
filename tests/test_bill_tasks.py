...
def test_sync_bill_enqueues_summary_votes_and_notifications(monkeypatch):
    first_db = object()
    ...
    assert notifications[1]["change_type"] == "status_changed"
    assert notifications[1]["extra"]["old_status"] == "pending"
    assert notifications[1]["extra"]["new_status"] == "approved"
    assert notifications[2]["change_type"] == "stage_changed"
